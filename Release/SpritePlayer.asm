...
;SpritePlayer.c:34: THIS->y--;
	...
	inc	hl
	ld	b,(hl)
;SpritePlayer.c:47: SPRITE_UNSET_VMIRROR(THIS);
	ld	hl,#0x000b
	add	hl,bc
	ld	c,l
	ld	b,h
	...
	ld	(bc),a
00106$:
;SpritePlayer.c:50: if(KEY_PRESSED(J_LEFT)){
	ld	hl,#_keys
	ld	c,(hl)
	ld	b,#0x00
	bit	1, c
	jp	Z,00108$
;SpritePlayer.c:51: THIS->x--;
	ld	hl,#_THIS + 1
	dec	hl
	ld	c,(hl)
	inc	hl
	...
	inc	hl
	ld	(hl),b
;SpritePlayer.c:52: SetSpriteAnim(THIS, anim_left, 10);
	ld	a,#0x0a
	push	af
	inc	sp
	ld	hl,#_anim_left
	...
	call	_SetSpriteAnim
	add	sp, #5
;SpritePlayer.c:53: TranslateSprite(THIS, -1 << delta_time, 0);
	ld	hl,#_delta_time
	ld	a,(hl)
	push	af
	ld	b,#0xff
...
00233$:
	...
	add	sp, #4
00108$:
;SpritePlayer.c:56: if(keys == 0){
	ld	hl,#_keys
	ld	a,(hl)
	or	a, a
	jr	NZ,00110$
;SpritePlayer.c:57: SetSpriteAnim(THIS, anim_idle, 20);
	ld	a,#0x14
	push	af
	inc	sp
	ld	hl,#_anim_idle
	...
	add	sp, #5
00110$:
;SpritePlayer.c:59: if (KEY_TICKED(J_A) && have_bomb>0) {
	ld	hl,#_keys
	ld	c,(hl)
	ld	b,#0x00
	bit	4, c
...
;SpritePlayer.c:34: THIS->y--;
	...
	inc	hl
	ld	b,(hl)
;SpritePlayer.c:60: SpriteManagerAdd(SPRITE_BOMB, THIS->x+15, THIS->y);
	ld	hl,#0x0009
	add	hl,bc
	ld	a,l
	ld	d,h
...
;SpritePlayer.c:61: have_bomb--;	
	...
	ld	(hl),d
00112$:
;SpritePlayer.c:70: SPRITEMANAGER_ITERATE(i, spr) {
...
00137$:
	...
	sub	a, c
	jp	Z,00139$
;SpritePlayer.c:71: if(spr->type == SPRITE_ENEMY) {
	pop	de
	push	de
	ld	hl,#0x0010
	add	hl,de
...
;SpritePlayer.c:72: if(CheckCollision(THIS, spr)) {
	...
	or	a, a
	jr	Z,00118$
;SpritePlayer.c:73: SetState(STATE_MENU);
	push	bc
	xor	a, a
	push	af
	inc	sp
	...
	inc	sp
	pop	bc
;SpritePlayer.c:74: PlayFx(CHANNEL_1, 10, 0x4f, 0xc7, 0xf3, 0x73, 0x86);
	push	bc
	ld	hl,#0x0086
	push	hl
	ld	l, #0x73
	...
	add	sp, #12
	pop	bc
;SpritePlayer.c:75: have_bomb = 3;
	ld	hl,#_have_bomb
	ld	(hl),#0x03
	inc	hl
	ld	(hl),#0x00
00118$:
;SpritePlayer.c:78: if(spr->type == SPRITE_ENEMY2) {
	ld	a,(bc)
	sub	a, #0x02
	jp	NZ,00122$
...
;SpritePlayer.c:81: PlayFx(CHANNEL_1, 10, 0x4f, 0xc7, 0xf3, 0x73, 0x86);
	...
	add	sp, #12
	pop	bc
;SpritePlayer.c:82: have_bomb = 3;
	ld	hl,#_have_bomb
	ld	(hl),#0x03
	inc	hl
	ld	(hl),#0x00
00122$:
;SpritePlayer.c:85: if (spr->type == SPRITE_BOMB) {
	ld	a,(bc)
	sub	a, #0x06
	jp	NZ,00126$
...
;SpritePlayer.c:88: PlayFx(CHANNEL_1, 10, 0x4f, 0xc7, 0xf3, 0x73, 0x86);
	...
	add	sp, #12
	pop	bc
;SpritePlayer.c:89: have_bomb = 3;
	ld	hl,#_have_bomb
	ld	(hl),#0x03
	inc	hl
	ld	(hl),#0x00
00126$:
;SpritePlayer.c:92: if (spr->type == SPRITE_KEY) {
	ld	a,(bc)
	sub	a, #0x03
	jp	NZ,00130$
;SpritePlayer.c:93: if (CheckCollision(THIS, spr)) {
	...
	or	a, a
	jp	Z,00130$
;SpritePlayer.c:94: SpriteManagerRemoveSprite(spr);
	push	bc
	pop	de
	pop	hl
	push	hl
	...
	add	sp, #2
	pop	bc
;SpritePlayer.c:95: SpriteManagerRemove(2); //need to place door always as 2 sprite
	push	bc
	ld	hl,#0x0002
	push	hl
	...
	add	sp, #2
	pop	bc
;SpritePlayer.c:96: SpriteManagerAdd(SPRITE_DOOROPEN, door_x, door_y);
	push	bc
	ld	hl,#_door_y
	ld	a,(hl+)
	ld	h,(hl)
	...
	add	sp, #5
	pop	bc
;SpritePlayer.c:97: PlayFx(CHANNEL_1, 10, 0x4U, 0xFEU, 0xA1U, 0x8FU, 0x86U);
	push	bc
	ld	hl,#0x0086
	push	hl
	ld	l, #0x8f
	...
	pop	bc
00130$:
;SpritePlayer.c:100: if (spr->type == SPRITE_DOOROPEN) {
	ld	a,(bc)
	sub	a, #0x05
	jr	NZ,00138$
;SpritePlayer.c:101: if (CheckCollision(THIS, spr)) {
	pop	hl
	push	hl
	push	hl
	ld	hl,#_THIS
	...
	or	a, a
	jr	Z,00138$
;SpritePlayer.c:102: SetState(next_lvl);
	ld	hl,#_next_lvl
	ld	b,(hl)
	push	bc
	inc	sp
...
00139$:
	add	sp, #5
	ret
;SpritePlayer.c:108: void Destroy_SPRITE_PLAYER() {
;	---------------------------------
