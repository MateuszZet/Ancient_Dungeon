...
;StateGame.c:29: have_bomb=2;
	...
	inc	hl
	ld	(hl),#0x00
;StateGame.c:31: if( have_diamond == 0 || have_diamond == 3 ){
...
;StateGame.c:33: scroll_target = SpriteManagerAdd(SPRITE_PLAYER, 20, 100);
	...
	inc	hl
	ld	(hl),d
;StateGame.c:34: door_x = 240;
	ld	hl,#_door_x
	ld	(hl),#0xf0
	inc	hl
	ld	(hl),#0x00
;StateGame.c:35: door_y = 112;
	ld	hl,#_door_y
	ld	(hl),#0x70
	inc	hl
	ld	(hl),#0x00
;StateGame.c:37: SpriteManagerAdd(SPRITE_KEY, 288, 112);
	ld	hl,#0x0070
	push	hl
	ld	hl,#0x0120
	push	hl
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:38: SpriteManagerAdd(SPRITE_DOOR, door_x, door_y);
	ld	hl,#_door_y
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:40: SpriteManagerAdd(SPRITE_ENEMY, 40, 32);
	ld	hl,#0x0020
	push	hl
	ld	l, #0x28
	push	hl
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:41: SpriteManagerAdd(SPRITE_ENEMY, 128, 80);
	ld	hl,#0x0050
	push	hl
	ld	l, #0x80
	push	hl
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:42: SpriteManagerAdd(SPRITE_ENEMY, 200, 112);
	ld	hl,#0x0070
	push	hl
	ld	l, #0xc8
	push	hl
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:43: SpriteManagerAdd(SPRITE_ENEMY, 288, 96);
	ld	hl,#0x0060
	push	hl
	ld	hl,#0x0120
	push	hl
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:44: SpriteManagerAdd(SPRITE_ENEMY3, 30, 30);	
	ld	hl,#0x001e
	push	hl
	ld	l, #0x1e
	push	hl
	...
	jp	00104$
00103$:
;StateGame.c:47: scroll_target = SpriteManagerAdd(SPRITE_PLAYER, 20, 100);
	ld	hl,#0x0064
	push	hl
	ld	l, #0x14
	push	hl
	...
	inc	hl
	ld	(hl),d
;StateGame.c:48: door_x = 240;
	ld	hl,#_door_x
	ld	(hl),#0xf0
	inc	hl
	ld	(hl),#0x00
;StateGame.c:49: door_y = 112;
	ld	hl,#_door_y
	ld	(hl),#0x70
	inc	hl
	ld	(hl),#0x00
;StateGame.c:51: SpriteManagerAdd(SPRITE_KEY, 288, 112);
	ld	hl,#0x0070
	push	hl
	ld	hl,#0x0120
	push	hl
	...
	call	_SpriteManagerAdd
	add	sp, #5
;StateGame.c:52: SpriteManagerAdd(SPRITE_DOOR, door_x, door_y);
	ld	hl,#_door_y
	ld	a,(hl+)
	ld	h,(hl)
	ld	l,a
	...
	add	sp, #5
00104$:
;StateGame.c:55: InitScrollTiles(0, 59, tiles, 3);
	ld	hl,#0x0000
	push	hl
	ld	a,#0x03
	push	af
	...
	add	sp, #7
;StateGame.c:56: InitScroll(mapWidth, mapHeight, map, collilision_tiles, 0, 3);
	ld	hl,#0x0000
	push	hl
	ld	a,#0x03
	push	af
...
;StateGame.c:57: SHOW_BKG;
	...
	ld	(hl),c
	ret
;StateGame.c:60: void Update_STATE_GAME() {
;	---------------------------------
; Function Update_STATE_GAME
; ---------------------------------
_Update_STATE_GAME::
;StateGame.c:61: if (KEY_PRESSED(J_START)) {
	ld	hl,#_keys
	ld	c,(hl)
	ld	b,#0x00
	bit	7, c
	ret	Z
;StateGame.c:62: SetState(STATE_GAME2);
	ld	a,#0x02
	push	af
	inc	sp
	call	_SetState
